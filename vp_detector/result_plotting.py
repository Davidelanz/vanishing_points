...
def show_em_result(datum, image_file, maxbest=4, true_vps=None,
    ...
    if target_size is not None:
        ...
        image = scipy.misc.imresize(
            image, resize_factor, interp='bilinear', mode=None)
    ...
    sphere_image = datum['sphere_image'] \
        if 'sphere_image' in datum else None
    ...
    if em_result is not None:
        ...
        plot_result(None, ax2, vps, angles, counts,
                    best_vps, img_size=sphere_image.shape[0])
        plot_result(None, ax3, vps, angles, counts,
                    best_vps, img_size=prediction.shape[0])

        if not (true_vps is None):
            angles_true = prob.calc_angles(true_vps.shape[0], true_vps)
            plot_result(None, ax2, true_vps, angles_true, None, None,
                        img_size=sphere_image.shape[0], std_mark='co')
            plot_result(None, ax3, true_vps, angles_true, None, None,
                        img_size=prediction.shape[0], std_mark='co')
        ...
        for li in range(ls.shape[0]):
            if vp_assoc[li] in best_vps:
                ...
                ax1.plot([lsc[li, 0], lsc[li, 2]], [
                         lsc[li, 1], lsc[li, 3]], c=color_val, lw=2)
            ...
            llen[li] = np.linalg.norm(
                [ls[li, 0] - ls[li, 2], ls[li, 1] - ls[li, 3]], ord=2)

        if horizon is not None:
            ax1.plot([horizon[0][0], horizon[1][0]], [
                     horizon[0][1], horizon[1][1]], c='c', lw=10)
...
def plot_result(ax1, ax2, vps, angles, vp_counts, best=None,
                img_size=250, std_mark='yo', markersize=1):
    ...
    for j in range(gvp.shape[0]):
        ...
        if not (ax1 is None):
            ax1.plot([gvp[j, 0]], [gvp[j, 1]], [gvp[j, 2]], mark,
                     markersize=markersize*np.minimum(
                np.maximum(pg[j] * scaling, 6), 20))
        if not (ax2 is None):
            angle = gang[j, :]
            pos = coconv.angle_to_index(angle, (img_size, img_size))
            ax2.plot(pos[0], img_size - 1 - pos[1], mark,
                     markersize=markersize*np.minimum(
                         np.maximum(pg[j] * scaling, 6), 20), alpha=0.6)
