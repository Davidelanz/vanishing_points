...
import numpy as np
from math import pi
from pylab import linspace, uint8
...
def sphere_line_plot(lines, size, alpha=0.1, f=1.0, alternative=False):
    ...
    for i in range(lines.shape[0]):
        ...
        if alternative:
            b = -np.arctan(
                -lines[i, 2] / (np.cos(a) * lines[i, 0] +
                                np.sin(a) * lines[i, 1]))
        else:
            b = -np.arctan((-lines[i, 0] * np.sin(a) -
                            lines[i, 2] * np.cos(a)) / lines[i, 1])
...
def plot_sphere_lines_to_tile(lines, im_size, filename, alpha=0.5, f=1.0,
                              alternative=False):
