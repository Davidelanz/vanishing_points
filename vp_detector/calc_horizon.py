...
def calculate_horizon_and_ortho_vp(
        em_result, maxbest=10, theta_vmin=np.pi/10., theta_z=np.pi/4.):
        ...
        for j in range(i, num_best):
            for k in range(j, num_best):
                if i != k and j != k and i != j:
                    combinations[combo_count, :] = np.array(
                        [i, j, k]).astype(int)
                    combo_count += 1
    ...
    if num_best > 2:
        ...
        for i in range(num_combinations):
            ...
            if np.abs(Va[1]) > np.abs(Vb[1]) \
                    and np.abs(Va[1]) > np.abs(Vc[1]):
                hVP1_temp = Vb
                hVP2_temp = Vc
                zVP_temp = Va
            ...
            elif np.abs(Vb[1]) > np.abs(Va[1]) \
                    and np.abs(Vb[1]) > np.abs(Vc[1]):
                hVP1_temp = Va
                hVP2_temp = Vc
                zVP_temp = Vb
            ...
            h1 = -l2
            h2 = l1
            h3 = ((v11*l2-v12*l1)/v13*(d2*h1Count) + (v21*l2-v22*l1) /
                  v23*(d1*h2Count)) / ((d1*h2Count)+(d2*h1Count))  # !!!

            hlin_temp = np.array([h1, h2, h3])

            # angle of the proposed horizon line:
            hvec = (hVP1_temp / hVP1_temp[2]) - (hVP2_temp / hVP2_temp[2])
            hang = np.arccos(np.abs(np.dot(hvec, np.array([1, 0, 0])))
                             / np.linalg.norm(hvec))
            ...
            # score for orthogonality of horizon, and vector between zenith
            # and principal point:
            ortho_score = 0
            if num_zenith == 1:
                cosphi = np.abs(
                    np.dot(hvec / np.linalg.norm(hvec), zenith
                           / np.linalg.norm(zenith)))
            ...
            score_dist[i] = 1 if (AB < costh and BC < costh and AC < costh
                                  and num_zenith == 1 and num_central <= 1 \
                                  # and hang < 30 * np.pi / 180 )
                                  # and min_count > 4 else 0
                                  and hang < 30 * np.pi / 180 \
                                  and zenithPos*horPos == 1) else 0
            # and hang < 30 * np.pi / 180)  else 0

            # sum of VP weights:
            score_weight[i] = counts[best_vps[a]] + \
                counts[best_vps[b]] + \
                counts[best_vps[c]]  # + 1*consistency_count
            ...
            if score[i] > 0:
                possible_solutions.append(
                    {"score": score[i], "zVP_id": zVP_id_temp,
                     "hVP1": hVP1_temp, "hVP2": hVP2_temp,
                     "h1Count": h1Count, "h2Count": h2Count,
                     "horizon": hlin_temp})
